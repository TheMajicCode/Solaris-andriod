...
script=script.replace(tail,`render=()=>{};updateRuntimeRegion=(el,html)=>{if(el)el.innerHTML=html};
window.__test={send,chat,composer,aiModeLabel,prepDescription,modelDetailsContent,chatFailureMessage,safeChatCode,errorContents,chatVerified,recordChatResult,resetChatProof,updateChatWait,
setFixture:next=>accept(next),setDraft:x=>{${sym('chatDraft')}=x},setRoute:x=>{${sym('route')}=x;${sym('navigationEpoch')}++},snapshot:()=>({${['waiting','chatDraft','privateEpoch','chatPendingId','error','errorCode'].map(n=>n+':'+sym(n)).join(',')},proof:chatVerified()}),
setContext:x=>{${sym('state')}.experience.localContext=x},setSub:x=>{${sym('sub')}=x},expireGrant:()=>{${sym('state')}.grants.expiresAt='2000-01-01T00:00:00Z'},setRuntime:s=>window.__solarisDailyStatus(s)};`);
const events={},calls=[],elements={};
...
setTimeout:(fn)=>{timers.push(fn);return timers.length},clearTimeout(){},setInterval:(fn)=>{intervals.push(fn);return intervals.length},clearInterval(){},
...
check('No new private storage writes or network bridge authority',()=>{assert(calls.every(c=>['solaris-legacy-read/1','solaris-vault-ui/1'].includes(c.transport)));assert(calls.filter(c=>c.transport==='solaris-vault-ui/1').every(c=>['converse','view'].includes(c.method)))});
