...
function conversationRequest(request, isConversation) {
  ...
  message.type = 'string';
  message.minLength = 1;
  message.maxLength = facts.length ? 400 : 320;
  ...
  var generation = {};
  generation.predict = facts.length ? 160 : 96;
  generation.temp = 0.3;
  ...
  if (!facts.length && !envelope.allowed.length) {
    ...
    system.content = 'You are Pocket LUCA. Give one specific, helpful sentence of at most 25 words, in ' + language + '. No diagnosis, treatment, invented facts or claims of saving. JSON: {"message":"your answer","sourceRefs":[]}. /no_think';
    history.push(system);
    if (Array.isArray(envelope.recent) && envelope.recent.length === 2 &&
        envelope.recent[0].role === 'user' && envelope.recent[1].role === 'assistant' &&
        typeof envelope.recent[0].content === 'string' && typeof envelope.recent[1].content === 'string') {
