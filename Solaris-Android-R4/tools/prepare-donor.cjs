 ...
 * bounded String.fromCharCode chunks. The exact compiled helper is VM-tested.
 */
const fs = require('fs');
  ...
  plugins: [function({types:t}) {
    function stringExpression(value) {
      ...
      for(let i=0; i<value.length; i+=12) {
        const args=[];
        for(let j=i; j<Math.min(i+12,value.length); j++) args.push(t.numericLiteral(value.charCodeAt(j)));
