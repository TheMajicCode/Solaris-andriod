...
function recentContext(task, owner, state) {
  ...
  var previous = owner.__solaris603Context;
  // The bridge preflights this compiler before converse. Keep a fixed baseline
  // per scope so repeating that read does not consume or advance the history.
  if (!previous || previous.scope !== scope) {
    var cursor = {};
    cursor.scope = scope;
    cursor.tail = tail;
    owner.__solaris603Context = cursor;
    return task;
  }
  if (previous.tail === tail || count < 2) return task;
  ...
  if (user.id === previous.tail || !Array.isArray(user.sources) || user.sources.length ||
      !Array.isArray(assistant.sources) || assistant.sources.length || !provenance ||
      provenance.authorityEpoch !== state.authorityEpoch || provenance.permissionRevision !== context.revision ||
      !Array.isArray(provenance.sourceRefs) || provenance.sourceRefs.length) return task;
  ...
  if (split < 0 || prompt.slice(-10) !== ' /no_think') return task;
  var envelope = JSON.parse(prompt.slice(split + 1, -10));
  if (envelope.facts.length || envelope.allowed.length) return task;
  envelope.recent = history;
