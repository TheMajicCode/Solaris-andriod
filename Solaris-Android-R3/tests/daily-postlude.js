...
(function () {
  ...
  (async function(){
    await test('valid greeting reaches real parser and persists valid state',async function(){
    ...
    var invalid=[
      {name:'plain greeting',output:'Hello',code:'CANDIDATE_INVALID'},
    ...
    await test('double preflight does not include an old completed pair on initial scope',async function(){
      ...
      preflight(f,'Hello');preflight(f,'Hello');assert(await invoke(f)===null,'initial greeting failed');assert(recentAt(f,0)===undefined,'old pair disclosed');assert(f.calls[0].prompt.indexOf('OLD_SYNTHETIC')===-1,'old text leaked');return {recentIncluded:false};
    });
