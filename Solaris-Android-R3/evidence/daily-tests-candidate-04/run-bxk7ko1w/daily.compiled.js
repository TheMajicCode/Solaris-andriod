...
(function () {
  ...
  _asyncToGenerator(function* () {
    ...
    for (var code of Array.from(new Set(directCodes.concat(sdkCodes.map(function (c) {
      return 'QVAC_' + c;
    }))))) {
    ...
    yield test('prior displayed message remains out of prompt', /*#__PURE__*/_asyncToGenerator(function* () {
      var f = fixture({
        prior: 'SYNTHETIC_HISTORY_SENTINEL'
      });
      assert((yield invoke(f)) === null, 'greeting failed');
      assert(f.calls[0].prompt.indexOf('SYNTHETIC_HISTORY_SENTINEL') === -1, 'unexpected new history disclosure');
      assert(f.service.state.conversation[0].id === f.before.conversation[0].id, 'prior ID changed');
      return {
    ...
    yield test('double preflight does not include an old completed pair on initial scope', /*#__PURE__*/_asyncToGenerator(function* () {
      var f = fixture({
        prior: 'OLD_SYNTHETIC_PAIR'
      });
      f.service.state.conversation.push({
        id: 'message_' + 'e'.repeat(32),
        role: 'assistant',
        text: 'OLD_SYNTHETIC_REPLY',
        at: '2026-09-16T00:00:00.000Z',
        mode: 'qvac-device',
        sources: [],
        eventCount: 1,
      ...
      preflight(f, 'Hello');
      preflight(f, 'Hello');
      assert((yield invoke(f)) === null, 'initial greeting failed');
      assert(recentAt(f, 0) === undefined, 'old pair disclosed');
      assert(f.calls[0].prompt.indexOf('OLD_SYNTHETIC') === -1, 'old text leaked');
      return {
        recentIncluded: false
      };
    }));
    yield test('double preflight retains newly completed source-free pair for next call', /*#__PURE__*/_asyncToGenerator(function* () {
      var f = fixture({
        output: '{"message":"Hello Nila","sourceRefs":[]}'
      });
      preflight(f, 'My name is Nila');
      assert((yield invoke(f, {
        message: 'My name is Nila'
      })) === null, 'first call');
      var saved = clone(f.service.state.conversation);
      preflight(f, 'What name did I give?');
      preflight(f, 'What name did I give?');
      assert((yield invoke(f, {
        message: 'What name did I give?',
        operation: 'synthetic_op_0002'
      })) === null, 'second call');
      var recent = recentAt(f, 1);
      assert(!!recent === !!globalThis.__solarisExpectedRecent, 'recent mode mismatch');
      if (recent) assert(JSON.stringify(recent) === JSON.stringify([{
        role: 'user',
        content: 'My name is Nila'
      }, {
        role: 'assistant',
        content: 'Hello Nila'
      }]), 'wrong recent pair');
      assert(JSON.stringify(f.service.state.conversation.slice(0, 2)) === JSON.stringify(saved), 'existing conversation mutated');
      return {
        recent: recent || null
      };
    }));
    yield test('third call includes only latest completed pair', /*#__PURE__*/_asyncToGenerator(function* () {
      var f = fixture();
      preflight(f, 'FIRST_SENTINEL');
      assert((yield invoke(f, {
        message: 'FIRST_SENTINEL'
      })) === null, 'first');
      preflight(f, 'SECOND_SENTINEL');
      assert((yield invoke(f, {
        message: 'SECOND_SENTINEL',
        operation: 'synthetic_op_0002'
      })) === null, 'second');
      preflight(f, 'THIRD_SENTINEL');
      preflight(f, 'THIRD_SENTINEL');
      assert((yield invoke(f, {
        message: 'THIRD_SENTINEL',
        operation: 'synthetic_op_0003'
      })) === null, 'third');
      var recent = recentAt(f, 2);
      assert(!!recent === !!globalThis.__solarisExpectedRecent, 'recent mode');
      if (recent) {
        assert(recent.length === 2 && recent[0].content === 'SECOND_SENTINEL', 'not latest pair');
        assert(f.calls[2].prompt.indexOf('FIRST_SENTINEL') === -1, 'older pair leaked');
      }
      return {
        recent: recent || null
      };
    }));
    for (var reset of ['session', 'authority', 'permissionRevision']) {
      yield test('recent context resets on ' + reset, /*#__PURE__*/_asyncToGenerator(function* () {
        var f = fixture();
        preflight(f, 'SCOPE_SENTINEL');
        assert((yield invoke(f, {
          message: 'SCOPE_SENTINEL'
        })) === null, 'first');
        if (reset === 'session') f.service.session++;else replaceState(f, function (s) {
          if (reset === 'authority') s.authorityEpoch++;else s.experience.localContext.revision++;
        });
        preflight(f, 'Hello again');
        preflight(f, 'Hello again');
        assert((yield invoke(f, {
          message: 'Hello again',
          operation: 'synthetic_op_0002'
        })) === null, 'second');
        assert(recentAt(f, 1) === undefined && f.calls[1].prompt.indexOf('SCOPE_SENTINEL') === -1, 'old scope disclosed');
        return {
          recentIncluded: false
        };
      }));
    }
    yield test('source-bearing previous answer is excluded from recent context', /*#__PURE__*/_asyncToGenerator(function* () {
      var f = fixture({
        profile: true,
        categories: ['profile'],
        output: '{"message":"Synthetic Profile","sourceRefs":["s1"]}'
      });
      preflight(f, 'What profile?', ['profile_owner']);
      assert((yield invoke(f, {
        message: 'What profile?',
        sources: ['profile_owner']
      })) === null, 'first');
      f.service.qvac.completeConversationTask = function (prompt, eligible) {
        f.calls.push({
          prompt: prompt,
          eligibleAtCall: eligible()
        });
      ...
      assert(recentAt(f, 1) === undefined && f.calls[1].prompt.indexOf('Synthetic Profile') === -1, 'private prior source disclosed');
      return {
        recentIncluded: false
      };
    }));
    yield test('selected current source prevents recent context addition', /*#__PURE__*/_asyncToGenerator(function* () {
      var f = fixture({
        profile: true,
        categories: ['profile']
      });
      preflight(f, 'SOURCEFREE_SENTINEL');
      assert((yield invoke(f, {
        message: 'SOURCEFREE_SENTINEL'
      })) === null, 'first');
      f.service.qvac.completeConversationTask = function (prompt, eligible) {
        f.calls.push({
          prompt: prompt,
      ...
      assert(recentAt(f, 1) === undefined && f.calls[1].prompt.indexOf('SOURCEFREE_SENTINEL') === -1, 'recent mixed with facts');
      return {
        recentIncluded: false
      };
    }));
    yield test('oversized recent pair dropped whole', /*#__PURE__*/_asyncToGenerator(function* () {
      var text = 'z'.repeat(230),
        f = fixture({
      ...
      assert(recentAt(f, 1) === undefined, 'oversized pair included');
      assert(f.calls[1].prompt.indexOf(text) === -1, 'partial history leaked');
      return {
        recentIncluded: false
      };
    }));
    yield test('recent pair dropped when it would exceed original prompt budget', /*#__PURE__*/_asyncToGenerator(function* () {
      var f = fixture();
      preflight(f, 'SHORT_SENTINEL');
      assert((yield invoke(f, {
        message: 'SHORT_SENTINEL'
      })) === null, 'first');
      var longMessage = 'b'.repeat(810);
      preflight(f, longMessage);
      assert((yield invoke(f, {
        message: longMessage,
        operation: 'synthetic_op_0002'
      })) === null, 'second');
      assert(recentAt(f, 1) === undefined, 'over-budget history included');
      assert(f.calls[1].prompt.length <= 1200, 'prompt exceeded budget');
      return {
        recentIncluded: false,
        promptBytes: f.calls[1].prompt.length
      };
    }));
    yield test('history numbers do not silently relax original grounding check', /*#__PURE__*/_asyncToGenerator(function* () {
      var f = fixture({
        output: '{"message":"Your temporary code is 42","sourceRefs":[]}'
      });
      preflight(f, 'My temporary code is 42');
      assert((yield invoke(f, {
        message: 'My temporary code is 42'
      })) === null, 'first');
      preflight(f, 'What was my code?');
      var error = yield invoke(f, {
        message: 'What was my code?',
        operation: 'synthetic_op_0002'
      });
      assert(error === expectedDiagnostic('CANDIDATE_UNGROUNDED'), 'grounding changed ' + error);
      assert(f.writes.length === 1 && f.service.state.conversation.length === 2, 'invalid follow-up persisted');
      return {
        actual: error,
        limitation: 'prior-turn numbers remain excluded from groundedNumbers'
      };
    }));
