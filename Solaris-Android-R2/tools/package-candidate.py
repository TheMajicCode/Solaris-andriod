...
BASELINE_APK = '228c3d9f282d716515e3640de2b13478a29e21c607293eaf589741ec6d34f183'
BASELINE_HBC = 'ac973292961cc7a1505a47a416ecafdff2e1688b7eac31f20571e503d74ec177'
PATCHED_HBC = 'fd8b38bcdd42421c18da4a0b69b2d58906ceddd7c4a2044b34faed51490f4653'
BASELINE_MANIFEST = '51967b0d46d2edac87ebfb40368a448ef790cd77fd8319daaeba5c463b4200e8'
CERTIFICATE = 'fac61745dc0903786fb9ede62a962b399f7348f0bb6f899b8332667591033b9c'
PACKAGE = 'org.solarishealth.edge.recovery'
OLD_VERSION, VERSION = '6.0.1-preview.sanctuary', '6.0.2-preview.sanctuary-guards'
OLD_CODE, CODE = 601, 602
...
def patch_manifest(data):
    ...
    require(struct.unpack_from('<I', data, 6712)[0] == OLD_CODE, 'AXML_VERSION_CODE')
    ...
    struct.pack_into('<I', result, 6712 + pool_growth, CODE)
    ...
    struct.pack_into('<I', original_nodes, 6712-(8+length), CODE)
    ...
    return bytes(result), {'stringIndex': 35, 'originalStringOffset': string_start,
                          ...
                          'newVersionCodeOffset': 6712 + pool_growth,
...
def main():
    ...
    require(file_sha(args.patched_hbc) == PATCHED_HBC, 'PATCHED_HBC_HASH')
    ...
    with zipfile.ZipFile(args.baseline) as source:
        ...
        (evidence / 'AndroidManifest-602.xml').write_bytes(manifest)
    ...
    expected_tree = original_tree.replace('versionCode(0x0101021b)=601', 'versionCode(0x0101021b)=602')
    expected_tree = expected_tree.replace('versionCode(0x0101021b)=0x259', 'versionCode(0x0101021b)=0x25a').replace(OLD_VERSION, VERSION)
    ...
    require("name='" + PACKAGE + "'" in badging and "versionCode='602'" in badging and
    ...
    report = {'format': 'solaris-v6-602-unsigned-packaging-or-verification/1',
              'baselineApkSha256': BASELINE_APK, 'candidatePath': str(candidate), 'candidateSha256': file_sha(candidate),
              'candidateBytes': candidate.stat().st_size, 'patchedHbcSha256': PATCHED_HBC,
              ...
              'scope': 'Two existing operation completion guards; full LUCA resume repair remains out of scope'}
