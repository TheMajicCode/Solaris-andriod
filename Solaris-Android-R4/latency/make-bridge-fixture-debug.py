...
def env_nodes(key,slots):
 ...
 return nodes
f0=env_nodes('module',range(20))+[I('CreateClosure',6,0,1),I('LoadConstUndefined',7),I('Call1',6,6,7),I('LoadConstUInt8',10,23),I('Call2',11,9,8,10),I('PutById',2,6,1,sid('receive')),I('Ret',7)]
f1=env_nodes('entries',range(34))+[I('CreateClosure',6,0,2),I('LoadConstUndefined',7),I('Call1',6,6,7),I('LoadConstUInt8',10,22),I('Call2',11,9,8,10),I('Ret',6)]
f2=[I('CreateEnvironment',0),I('LoadConstUInt8',10,20),I('Call2',11,9,8,10),I('CreateGeneratorClosure',0,0,6928),I('LoadConstUInt8',10,21),I('Call2',11,9,8,10),I('Ret',0)]
mutable=bytearray(data[:-20]);changed=[]
for fid,nodes,envsize in [(0,f0,20),(1,f1,34),(2,f2,0)]:
 nodes=[I('GetGlobalObject',8),I('GetById',9,8,3,sid('print')),I('LoadConstUInt8',10,fid),I('Call2',11,9,8,10)]+nodes
 body,_=assemble(r,nodes);h=r.get_small_func_header_reader().from_buffer_copy(bytes(r.function_headers[fid]));before=fields(h)
 h.offset=len(mutable);h.bytecodeSizeInBytes=len(body);h.environmentSize=envsize;h.frameSize=20;h.highestReadCacheIndex=3;h.highestWriteCacheIndex=1;h.paramCount=1;h.hasExceptionHandler=0;h.hasDebugInfo=0
