...
for p in sorted((root/'fixtures').glob('*.json')):
 ...
 (root/'native-summary.json').write_text(json.dumps({'scope':'Linux native addon only; exact pinned model; no Android RPC or phone performance claim','results':results},indent=2))
