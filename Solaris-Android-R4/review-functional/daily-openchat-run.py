...
root=pathlib.Path(__file__).resolve().parent; workspace=root.parents[1]
...
subprocess.run(['node',str(tool),str(root/'daily-openchat-postlude.js'),str(compiled)],check=True,capture_output=True,text=True)
