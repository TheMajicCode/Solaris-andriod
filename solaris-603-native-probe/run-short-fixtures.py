...
results=[]
for p in sorted((root/'fixtures').glob('short-*.json')):
 start=time.time()
 try:
 ...
 (root/'results/short-summary.json').write_text(json.dumps({'scope':'Linux native addon only, same pinned model, no Android RPC or phone performance claim','results':results},indent=2))
